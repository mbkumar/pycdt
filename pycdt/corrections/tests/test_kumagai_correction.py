...
class KumagaiBulkInitANDCorrectionTest(unittest.TestCase): #also might want to test outcar Kumagai method...
    def setUp(self):
    ...
    def test_reciprocal_sum(self): #this is initialized with KBI. Not sure how else to test besides size of list of vectors
    ...
    def test_plot(self):
        tmpforplot={'C':{'r':[1,2,3],'Vqb':[0.1,0.2,0.3],
                         'Vpc':[-0.05,-0.15,-0.25]},
                    'EXTRA':{'wsrad':1.,'potalign':.05,'lengths':(3,3,3)}}
    ...
    def test_get_sum_at_r(self):
        val = get_g_sum_at_r(self.kbi.g_sum, self.bs, self.bl.dim, [0.1,0.1,0.1])
        self.assertAlmostEqual(val, 0.04795055159361078)

    def test_anisotropic_madelung_potential(self):
        val = anisotropic_madelung_potential(self.bs, self.bl.dim, self.kbi.g_sum, [0.1,0.1,0.1],
                                              [[15.,.1,-.1],[.1,13,0],[-.1,0,20]], -3,
                                              self.kbi.gamma, self.kbi.tolerance)
        self.assertAlmostEqual(val, -4.2923511216202419)

    def test_anisotropic_pc_energy(self):
        val = anisotropic_pc_energy(self.bs, self.kbi.g_sum,
                                    [[15.,.1,-.1],[.1,13,0],[-.1,0,20]], -3,
                                    self.kbi.gamma, self.kbi.tolerance)
...
class KumagaiSetupFunctionsTest(unittest.TestCase):
    ...
    def test_kumagai_init(self):
        angset, bohrset, vol, determ, invdiel = \
            kumagai_init(self.bs, [[15.,.1,-.1],[.1,13,0],[-.1,0,20]])
    ...
    def test_real_sum(self):
        ...
        tmpdiel=[[15.,.1,-.1],[.1,13,0],[-.1,0,20]]
        val = real_sum(a,b,c,np.array([.1,.1,.1]),-1,tmpdiel,3.,1)
        self.assertAlmostEqual(val, -0.0049704211394050414)

    def test_disttrans(self): #not sure what best way to test dictionary is...
        pass

    def test_wigner_seitz_radius(self):
        self.assertAlmostEqual(wigner_seitz_radius(self.bs), 2.8750914999999999)

    def test_read_ES_avg(self): #not sure what best way to test dictionary is...
        pass

    def test_read_ES_avg_fromlocpot(self): #not sure what best way to test dictionary is...
        pass
...
class StructureFunctionsTest(unittest.TestCase):
    ...
    def test_find_defect_pos(self): #might be good to include more defect poscars to test all types of defect finding..
        bdefect,ddefect = find_defect_pos(self.bs,self.ds)
        self.assertEqual(list(bdefect),[ 0.,  0.,  0.])
